...
find "$TARGET_DIR" -type f -iname "*.mp3" -print0 | while IFS= read -r -d '' file; do
    ...
    # MODIFICA: Rimuove "Artista - Album - Traccia - " dall'inizio del nome del file.
    # Se il file non segue questo pattern rigido, rimuove comunque il numero di traccia iniziale come prima.
    if echo "$titolo_grezzo" | grep -q " - "; then
        # Cattura tutto ciò che si trova dopo l'ultimo blocco " - [Numero] - "
        titolo_pulito=$(echo "$titolo_grezzo" | sed -E 's/^.* - +[0-9]+ +- +//' | xargs)
    else
        # Fallback: se il file si chiama solo "13 - The Madness..." o "13-The Madness..."
